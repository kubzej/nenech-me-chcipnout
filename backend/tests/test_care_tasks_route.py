...
def test_copy_sections_for_legacy_weather_protection_task_use_fields() -> None:
    assert _copy_sections_for_task(
        {
            "task_type": "weather_protection",
            "explanation": "Old combined text should not be used.",
            "recommendation_json": {
                "forecast_temp_max_c": 34.2,
                "heat_sensitive_above_c": 30,
            },
        }
    ) == [
...
def test_to_care_task_item_uses_survival_hint_for_legacy_weather_instruction() -> None:
    task = _to_care_task_item(
        {
            ...
            "completed_at": None,
            "outcome_note": None,
            "recommendation_json": {"forecast_temp_max_c": 34.2},
            "kytky": {
                "care_profiles": {
    ...
    assert task.instructions == "V horku zalij ráno nebo večer."
    assert [section.model_dump() for section in task.copy_sections] == [
        {"kind": "weather", "text": "Dnes bude 34.2 °C."},
        {"kind": "action", "text": "Dej ji do stínu a zkontroluj zálivku."},
    ]
