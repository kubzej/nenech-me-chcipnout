-- Complete one or more care tasks atomically.
--
-- Watering/fertilizing tasks are container-scoped in care_events, so a grouped
-- "zalito" action for several Kytky in one container must create one container
-- event and mark all task rows done. Everything else creates one event per task.
...
begin
  ...
  into v_task_count, v_workspace_id, v_distinct_workspace_count
  from public.care_tasks
  where id = any(p_task_ids)
    and status = 'pending';
  ...
  if v_distinct_workspace_count <> 1 or not public.is_workspace_member(v_workspace_id) then
