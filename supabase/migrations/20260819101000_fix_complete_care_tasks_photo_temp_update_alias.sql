-- Qualify the temporary table update from the previous RPC fix. The function
-- returns a task_id column, so an unqualified task_id in PL/pgSQL is ambiguous.
...
begin
  ...
  if p_photo_storage_path is not null then
    ...
    update completed_task_rows as completed
    set photo_id = v_photo_id
    where completed.task_id = any(p_task_ids);
  end if;
